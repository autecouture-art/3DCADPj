# 3D CAD Suite

包括的な3D CADアプリケーション - ビューアー、モデリングツール、ファイルコンバーターを統合したWebベースのCADシステム

## 機能

### 1. 3D CADビューアー 👁️
- インタラクティブな3Dビューアー
- CADファイルの読み込みと表示
- カメラコントロール（回転、パン、ズーム）
- グリッドと座標軸の表示/非表示
- オブジェクトの選択と管理

### 2. 3Dモデリングツール 🔨
- 基本図形の作成
  - 立方体
  - 球
  - 円柱
  - 円錐
  - トーラス
- CAD操作
  - 押し出し
  - 回転体
  - ブーリアン演算（結合、減算）
  - フィレット
- スケッチツール
  - 線
  - 円
  - 矩形
  - ポリゴン

### 3. CADファイルコンバーター 🔄

#### 完全サポート ✅
- **STL** (Stereolithography) - 3Dプリント用標準形式
- **OBJ** (Wavefront) - 汎用3Dモデル形式
- **PLY** (Polygon File Format) - 点群・スキャンデータ用
- **FBX** (Filmbox) - アニメーション対応形式
- **glTF/GLB** (GL Transmission Format) - Web用3D標準

#### 基本サポート（読み込み＆エクスポート） ⚡
- **STEP** (ISO 10303) - 産業用CAD標準
  - 基本的なSTEPファイルの読み込みとエクスポートに対応
  - 複雑な形状は簡略化される場合があります

#### SolidWorksファイル対応 📋
- **SolidWorksファイル** (.sldprt, .sldasm, .slddrw)
  - 直接読み込みは不可（プロプライエタリ形式のため）
  - **推奨**: SolidWorksでSTEP形式にエクスポート後、アップロード
  - アプリ内で変換ガイドを表示

#### eDrawingsファイル対応 📐
- **eDrawingsファイル** (.eprt, .easm, .edrw, .edrawings)
  - 直接読み込みは不可（プロプライエタリ形式のため）
  - **推奨**: eDrawings Viewerまたは元のCADソフトウェアでSTEP形式にエクスポート後、アップロード
  - アプリ内で変換ガイドを表示

#### 開発中 🔮
- **IGES** (Initial Graphics Exchange) - CAD交換形式
- **DXF** (Drawing Exchange Format) - AutoCAD図面形式

#### 機能
- 複数フォーマット間の相互変換
- リアルタイム変換進捗表示
- 変換履歴の管理
- ダウンロード機能

### 4. CADツール 🛠️

#### 計測ツール
- 距離測定
- 角度測定
- 体積計算
- 表面積計算
- 重心計算
- バウンディングボックス

#### 解析ツール
- ジオメトリチェック
- 干渉チェック
- 統計情報
- 肉厚解析
- 曲率解析
- ドラフト角度解析

#### エクスポートツール
- **STL/OBJ/glTF/PLY/STEPエクスポート** ✅ 実装済み
- 画像エクスポート（計画中）
- アニメーション出力（計画中）
- 技術図面出力（計画中）

#### ユーティリティ
- メッシュ修復
- メッシュ簡略化
- メッシュ平滑化
- 座標系変換
- 単位変換
- バッチ処理

### 5. モバイル対応 📱

#### iPhone / Androidスマートフォン対応 ✅
このアプリケーションは**iPhoneやAndroidスマートフォンでも使用可能**です！

**自動最適化機能**:
- **自動検出**: iPhone・Androidスマートフォンを自動的に検出
- **ビューワーモード**: モバイルデバイスでは自動的に3D CADビューワーモードで起動
- **シンプルUI**: モバイル向けに簡略化されたインターフェース
- **タッチ操作**: タッチジェスチャーに最適化
  - **ピンチ**: ズームイン/ズームアウト
  - **スワイプ**: 3Dモデルの回転
  - **ドラッグ**: カメラの移動

**推奨用途**:
- ✅ **3Dモデルの閲覧**: CADファイルを開いて確認
- ✅ **外出先での確認**: 現場やクライアント先での3Dモデル表示
- ✅ **簡易プレビュー**: ファイルの内容を素早く確認
- ⚠️ **編集作業**: 複雑な編集はPC/Macを推奨
- ⚠️ **大容量ファイル**: パフォーマンスの観点からPC/Macを推奨

**対応デバイス**:
- iPhone（iOS Safari）
- Android（Chrome）
- タブレット端末（iPad、Androidタブレット）

**注意事項**:
- 大きなCADファイルはモバイルデバイスでは動作が遅くなる場合があります
- 複雑なモデリング作業はデスクトップ環境を推奨します
- バッテリー消費にご注意ください

## 技術スタック

- **フロントエンド**: React 18 + TypeScript
- **3Dレンダリング**: Three.js + React Three Fiber
- **状態管理**: Zustand
- **ビルドツール**: Vite
- **スタイリング**: CSS Modules

## セットアップ

### 必要条件
- Node.js 18以上
- npm または yarn

### インストール

```bash
# 依存関係のインストール
npm install

# 開発サーバーの起動
npm run dev

# 本番ビルド
npm run build

# プレビュー
npm run preview
```

## 使用方法

### 起動
```bash
npm run dev
```

ブラウザで `http://localhost:3000` を開きます。

### 基本操作

#### ビューアーモード
1. 「ビューアー」タブをクリック
2. 「CADファイルを開く」ボタンでファイルを読み込み
3. マウスで3Dビューを操作
   - 左クリック＋ドラッグ: 回転
   - 右クリック＋ドラッグ: パン
   - マウスホイール: ズーム

#### モデラーモード
1. 「モデラー」タブをクリック
2. 左側のツールバーから図形を選択
3. クリックして3D空間に配置
4. オブジェクトをクリックして選択・編集

#### コンバーターモード
1. 「コンバーター」タブをクリック
2. 「ファイルを選択」ボタンでファイルをアップロード
3. 変換元と変換先のフォーマットを選択
4. 「変換を開始」ボタンをクリック
5. 完了後、「ダウンロード」ボタンでファイルを保存

#### ツールモード
1. 「ツール」タブをクリック
2. カテゴリ（計測、解析、エクスポート、ユーティリティ）を選択
3. 使用したいツールをクリック

## プロジェクト構造

```
3DCADPj/
├── src/
│   ├── components/
│   │   ├── Toolbar/          # メインツールバー
│   │   ├── Sidebar/          # オブジェクトリスト
│   │   ├── Viewer/           # 3Dビューアー
│   │   ├── Modeler/          # モデリングツール
│   │   ├── Converter/        # ファイルコンバーター
│   │   └── Tools/            # CADツール
│   ├── store/                # Zustand状態管理
│   ├── types/                # TypeScript型定義
│   ├── App.tsx               # メインアプリケーション
│   ├── main.tsx              # エントリーポイント
│   └── index.css             # グローバルスタイル
├── index.html
├── package.json
├── tsconfig.json
├── vite.config.ts
└── README.md
```

## 開発

### コーディング規約
- TypeScriptの厳格モードを使用
- ESLintルールに従う
- コンポーネントは機能ごとにディレクトリで整理

### 新機能の追加
1. `src/components/`に新しいコンポーネントを作成
2. 必要に応じて`src/store/useAppStore.ts`に状態を追加
3. `src/types/index.ts`に型定義を追加

## ライセンス

MIT License

## 貢献

プルリクエストを歓迎します。大きな変更の場合は、まずissueを開いて変更内容を議論してください。

## 今後の開発予定

### 高優先度
- [ ] STEP/IGES/DXFファイル対応（opencascade.js統合の改善）
- [ ] ブーリアン演算の完全実装
- [ ] 画像・アニメーションエクスポート機能

### 中優先度
- [ ] より高度なモデリング機能（フィレット、シェル化）
- [ ] クラウド保存機能
- [ ] プラグインシステム

### 低優先度
- [ ] コラボレーション機能
- [ ] AR/VRビューアー

### 完了済み ✅
- [x] 基本的なモバイル対応（iPhone/Android自動検出、ビューワーモード最適化）

## サポート

問題が発生した場合は、GitHubのIssuesセクションで報告してください。
